...
task main()
{
	...
	repeat (forever) {
		...
		if (getCompetitionAutonomous() == true) {
			...
			setMultipleMotors(75, port6, port7);
			wait(1, seconds);
			stopMultipleMotors(port6, port7);
			// Go Backwards
		...
		if (getCompetitionDriverControl() == true) {
			repeat (forever) {
				...
				if (vexRT[Btn6U] == 1) {
					setMultipleMotors(127, port3, port5);
				} else {
					if (vexRT[Btn6D] == 1) {
						setMultipleMotors(-127, port3, port5);
					} else {
						if (vexRT[Btn8U] == 1) {
							setMultipleMotors(75, port3, port5);
						} else {
							if (vexRT[Btn8D] == 1) {
								setMultipleMotors(-75, port3, port5);
							} else {
								setMultipleMotors(0, port3, port5);
							}
						}
					}
				}
				if (vexRT[Btn5U] == 1) {
					setMultipleMotors(127, port2, port4);
				} else {
					if (vexRT[Btn5D] == 1) {
						setMultipleMotors(-127, port2, port4);
					} else {
						if (vexRT[Btn7U] == 1) {
							setMultipleMotors(75, port2, port4);
						} else {
							if (vexRT[Btn7D] == 1) {
								setMultipleMotors(-75, port2, port4);
							} else {
								setMultipleMotors(0, port2, port4);
							}
						}
					}
				}
				// Intake
				if (vexRT[Ch2] > 15) {
					setMultipleMotors(y, leftIntake, rightIntake);
					y = vexRT[Ch2] * 0.5;
					wait(1, milliseconds);
				} else {
					if (vexRT[Ch2] < -15) {
						...
						setMultipleMotors(0, leftIntake, rightIntake);
					}
				}
				// Arm control
